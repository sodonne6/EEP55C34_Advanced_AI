...
class SignFeatsDataset(FairseqDataset):
    ...
    def __getitem__(self, index):
        ...
        # ResNet50 with MediaPipe
        if self.feats_type == SignFeatsType.resnet50:
            with open(feats_file, "rb") as f:
                resnet = np.load(f)  # (T, 2048, 1, 1) or (T, 2048)

            # Flatten ResNet from (T, 2048, 1, 1) to (T, 2048)
            if resnet.ndim == 4:
                resnet = resnet.squeeze(-1).squeeze(-1)
            elif resnet.ndim == 3:
                resnet = resnet.squeeze(-1)

            with open(feats_mediapipe_file, "rb") as f:
                mediapipe = np.load(f)  # (T, 33, 3) pose landmarks

            # Align timelines
            T_common = min(resnet.shape[0], mediapipe.shape[0])
            start = offset if 0 <= offset < T_common else 0
            end = min(start + length, T_common) if length > 0 else T_common
            if end <= start:
                start, end = 0, T_common

            resnet = resnet[start:end]       # (T', 2048)
            mediapipe = mediapipe[start:end] # (T', 33, 3)

            resnet = self.postprocess_array(resnet, kind="resnet50")
            mediapipe = self.postprocess_array(mediapipe, kind="mediapipe_npy")

            return {"id": index, "vid_id": _id, "source": resnet, "mediapipe_source": mediapipe}
        ...
        # I3D or OpenPose with MediaPipe
        elif self.feats_type in (SignFeatsType.i3d, SignFeatsType.openpose):
            with open(feats_file, "rb") as f:
                pose = np.load(f)  # (T, 1024)

            with open(feats_mediapipe_file, "rb") as f:
                pose_mediapipe = np.load(f)  # (T, 33, 3)
    ...
    def postprocess_array(self, arr: np.ndarray, kind: str):
        ...
        Use for i3d/openpose/resnet AND mediapipe .npy.
        """
        ...
        for sample in samples:
            ...
            if self.feats_type == SignFeatsType.mediapipe:
                ...
                if mp.dim() == 3:  # (T, 33, 3)
                    ...
                    collated_mediapipe_sources.append(F.pad(mp, (0, 0, 0, 0, 0, 0, 0, diff_mp_length), value=0))
