...
class Sign2TextTransformerEncoder(FairseqEncoder):
    """Sign-to-text Transformer encoder + SGCN branch with gated residual fusion."""
    ...
    def __init__(self, cfg, feats_type: SignFeatsType, feat_dim: int):
        ...
        # Gated residual fusion: x + g * pose_proj(x2)
        self.pose_proj = nn.Linear(self.encoder2.output_dim, cfg.encoder_embed_dim)
        self.gate_proj = nn.Linear(cfg.encoder_embed_dim * 2, cfg.encoder_embed_dim)
    ...
    def forward(self, src_tokens, src_mediapipe_tokens, encoder_padding_mask, mediapipe_padding_mask, return_all_hiddens=False):
        ...
        pose_aligned = self.pose_proj(x2)                     # (T,B,C)
        gate_input = torch.cat([x, pose_aligned], dim=2)     # (T,B,2C)
        gate = torch.sigmoid(self.gate_proj(gate_input))     # (T,B,C)
        fused_output = x + gate * pose_aligned               # (T,B,C)
