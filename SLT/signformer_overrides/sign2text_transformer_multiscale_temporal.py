...
class Sign2TextTransformerEncoder(FairseqEncoder):
    """Sign-to-text Transformer encoder + multi-scale temporal SGCN branch."""
    ...
    def __init__(self, cfg, feats_type: SignFeatsType, feat_dim: int):
        ...
        # Skeleton encoder with dual temporal streams:
        # raw coordinates + first-order temporal difference (velocity)
        pose_graph = Graph(33)
        self.encoder2_raw = StableSgcnLstm(33, pose_graph.AD, pose_graph.AD2)
        self.encoder2_vel = StableSgcnLstm(33, pose_graph.AD, pose_graph.AD2)

        self.skel_fuse = nn.Sequential(
            nn.Linear(self.encoder2_raw.output_dim * 2, self.encoder2_raw.output_dim),
            nn.ReLU(),
    ...
    def forward(self, src_tokens, src_mediapipe_tokens, encoder_padding_mask, mediapipe_padding_mask, return_all_hiddens=False):
        ...
        # Temporal velocity stream: v_t = x_t - x_{t-1}, v_0 = 0
        mp_vel = torch.zeros_like(src_mediapipe_tokens[:, :, 0:33, :])
        mp_vel[:, 1:, :, :] = src_mediapipe_tokens[:, 1:, 0:33, :] - src_mediapipe_tokens[:, :-1, 0:33, :]
        mp_vel = mp_vel.permute(0, 3, 2, 1).contiguous()  # (B,3,33,T)

        if self.num_updates < 1:
            print("DEBUG mp_vel(for sgcn):", mp_vel.shape, mp_vel.dtype, mp_vel.device)

        # ensure sgcn module on same device
        #self.encoder2_raw = self.encoder2_raw.to(mp.device)
        #self.encoder2_vel = self.encoder2_vel.to(mp_vel.device)
        #self.fuse_proj = self.fuse_proj.to(mp.device)

        x2_raw = self.encoder2_raw(mp)
        x2_vel = self.encoder2_vel(mp_vel)
        x2 = self.skel_fuse(torch.cat([x2_raw, x2_vel], dim=-1))
